...
void SampleWindow::create() {
	...
	if (m_renderer) {
		...
		m_camera = new SpatialCamera(projectionType, Right, m_renderer->getProjectionSize());
		if (m_camera->getProjectionType() == Orthographic) {
			...
			m_camera->setFrustum(l, r, b, t, 0.01f, 500.0f);
		} else if (m_camera->getProjectionType() == Perspective) {
			m_camera->setFrame(Vec3(0.0f, 47.5f, 47.5f), Vec3(0, -50, -100));
			m_camera->setFrustum(120.0f, float(m_xSize) / float(m_ySize), 0.01f, 1000.0f);
		}
