...
void Bvh::collectTreeSpatialBounds(std::list<SpatialBound3*>& spatialBounds) {
	if (subtreesArePresent()) {
		for (int i = 0; i < 2; ++i) {
			if (true) {
...
void Bvh::collectNodeSpatialBounds(std::list<SpatialBound3*>& spatialBounds) {
...
void Bvh::collectTreeSpatialBounds(std::list<SpatialBound3*>& spatialBounds, const Frustum3& frustum) {
	if (subtreesArePresent()) {
		for (int i = 0; i < 2; ++i) {
...
void Bvh::collectNodeSpatialBounds(std::list<SpatialBound3*>& spatialBounds, const Frustum3& frustum) {
	for (SpatialBound3* acceptedSpatial : m_acceptedSpatials) {
...
void Bvh::collectTreeBounds(std::list<Bound3>& bounds) {
	if (subtreesArePresent()) {
		for (int i = 0; i < 2; ++i) {
			if (true) {
...
void Bvh::collectNodeBounds(std::list<Bound3>& bounds) {
...
void Bvh::collectTreeBounds(std::list<Bound3>& bounds, const Frustum3& frustum) {
	if (subtreesArePresent()) {
		for (int i = 0; i < 2; ++i) {
			if (testIntersect(m_children[i]->m_boundingBox, frustum)) {
...
void Bvh::collectNodeBounds(std::list<Bound3>& bounds, const Frustum3& frustum) {
	Bound3 currentBound = getBoundingBox();
