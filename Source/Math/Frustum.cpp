...
void Frustum3::setMatrix(const ProjectionSize& projectionSize, const Mat4& matrixO, const bool& reverseZ) {
	Mat4 invndc = projectionSize == X2Y2Z2 ? Mat4::identity : Mat4::translate(Vec3(0.0f, 0.0f, -1.0f)) * Mat4::scale(Vec3(1.0f, 1.0f, 2.0f));
...
void Frustum3::set(const Mat4& cameraTransformMatrix, const ProjectionParameter& projection, const bool& infinite) {
	...
	float halfWidthNear = projection.n * (projection.rTangent - projection.lTangent) * 0.5f;
	float halfHeightNear = projection.n * (projection.tTangent - projection.bTangent) * 0.5f;
	...
	float halfWidthFar = projection.f * (projection.rTangent - projection.lTangent) * 0.5f;
	float halfHeightFar = projection.f * (projection.tTangent - projection.bTangent) * 0.5f;
