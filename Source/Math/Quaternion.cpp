...
Quat::Quat(const Mat3& matrix) {
	float t = matrix.d[0][0] + matrix.d[1][1] + matrix.d[2][2];
	if (t >= 0.0) {
		float r = 2 * sqrt(1 + t);
		float rinv = 1.0f / (r);
		x = rinv * (matrix.d[1][2] - matrix.d[2][1]);
		y = rinv * (matrix.d[2][0] - matrix.d[0][2]);
		z = rinv * (matrix.d[0][1] - matrix.d[1][0]);
		w = r * 0.25f;
	} else if (matrix.d[0][0] >= matrix.d[1][1] && matrix.d[0][0] >= matrix.d[2][2]) {
		float r = 2 * sqrt(1 - matrix.d[1][1] - matrix.d[2][2] + matrix.d[0][0]);
		float rinv = 1.0f / (r);
		x = r * 0.25f;
		y = rinv * (matrix.d[0][1] + matrix.d[1][0]);
		z = rinv * (matrix.d[0][2] + matrix.d[2][0]);
		w = rinv * (matrix.d[1][2] - matrix.d[2][1]);
	} else if (matrix.d[1][1] >= matrix.d[2][2]) {
		float r = 2 * sqrt(1 - matrix.d[0][0] - matrix.d[2][2] + matrix.d[1][1]);
		float rinv = 1.0f / (r);
		x = rinv * (matrix.d[0][1] + matrix.d[1][0]);
		y = r * 0.25f;
		z = rinv * (matrix.d[1][2] + matrix.d[2][1]);
		w = rinv * (matrix.d[2][0] - matrix.d[0][2]);
	} else {
		float r = 2 * sqrt(1 - matrix.d[0][0] - matrix.d[1][1] + matrix.d[2][2]);
		float rinv = 1.0f / (r);
		x = rinv * (matrix.d[0][2] + matrix.d[2][0]);
		y = rinv * (matrix.d[1][2] + matrix.d[2][1]);
		z = r * 0.25f;
		w = rinv * (matrix.d[0][1] - matrix.d[1][0]);
	}
}
...
Quat::Quat(const Vec3& r, const Vec3& u, const Vec3& b) {
...
bool almostEqual(const Quat& u, const Quat& v) {
	return almostEquals(u.x, v.x) && almostEquals(u.y, v.y) && almostEquals(u.z, v.z) && almostEquals(u.w, v.w);
}

Quat lookat(const Vec3& direction) {
	Vec3 b = direction.normal();
	Vec3 r = (cross(Vec3(0.0f, 1.0f, 0.0f), b)).normal();
