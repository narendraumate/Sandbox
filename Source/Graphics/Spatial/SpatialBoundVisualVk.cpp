...
SpatialBoundVisualVk3::SpatialBoundVisualVk3(const Mesh& mesh, const Material& material)
: SpatialBoundVisual3(mesh, material) {
	...
	if (mesh.primitives.at(0).points.indices.size()) {
		m_indexBuffers[0] = new IndexBufferVk(IndexBuffer::Point, mesh.primitives.at(0).points.indices);
	}
	if (mesh.primitives.at(0).edges.indices.size()) {
		m_indexBuffers[1] = new IndexBufferVk(IndexBuffer::Edge, mesh.primitives.at(0).edges.indices);
	}
	if (mesh.primitives.at(0).triangles.indices.size()) {
		m_indexBuffers[2] = new IndexBufferVk(IndexBuffer::Triangle, mesh.primitives.at(0).triangles.indices);
	}
	if (mesh.primitives.at(0).quadrilaterals.indices.size()) {
		m_indexBuffers[3] = new IndexBufferVk(IndexBuffer::Quadrilateral, mesh.primitives.at(0).quadrilaterals.indices);
	}
	if (mesh.primitives.at(0).faces.indices.size()) {
		m_indexBuffers[4] = new IndexBufferVk(IndexBuffer::Face, mesh.primitives.at(0).faces.indices, mesh.primitives.at(0).faces.counts);
	}
