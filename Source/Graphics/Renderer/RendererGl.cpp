...
void RendererGl::beginFrame() {
	ProjectionDirectionRestraint projectionRestraint = m_projectionDirectionRestraint;
	if (projectionRestraint == ForwardFinite || projectionRestraint == ForwardInfinite) {
		glDepthFunc(GL_LESS);
...
void RendererGl::draw(SpatialCamera* camera, std::list<SpatialBound3*> bounds, std::vector<VisualEffect*> visualEffects, const std::vector<SpatialLight*>& spatialLights) {
	...
	for (auto bound : bounds) {
		...
		if (visualMaterialGl && visualEffectGl && vertexBufferGl && visualIndexBufferCount) {
			...
			ProjectionDirectionRestraint projectionRestraint = m_projectionDirectionRestraint;
			int rangeZeroToOne = projectionRestraint == ForwardFinite || projectionRestraint == ForwardInfinite;
