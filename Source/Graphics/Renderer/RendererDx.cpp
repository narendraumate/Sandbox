...
SpatialBoundVisual3* RendererDx::createVisualSpatial(const Mesh& mesh, const Material& material) {
	return new SpatialBoundVisualDx3(mesh, material);
}
...
void RendererDx::draw(SpatialBoundVisual3* visual, VisualEffect* visualEffect, const Vec3& eyePosition, const std::vector<SpatialLight*>& spatialLights) {
}
...
VisualScreenSpace* RendererDx::createVisualScreenSpace(const Effect& effect) {
	return new DxVisualScreenSpace(effect);
}
...
void RendererDx::destroyVisualScreenSpace(VisualScreenSpace* visualScreenSpace) {
	DxVisualScreenSpace* dxVisualScreenSpace = dynamic_cast<DxVisualScreenSpace*>(visualScreenSpace);
	delete dxVisualScreenSpace;
}

void RendererDx::draw(const Bound3& bound, VisualEffect* visualEffect, VisualMaterial* visualMaterial, const Mat4& worldMatrix, const Mat4& viewMatrix, const Mat4& viewProjectionMatrix, const Vec3& lightCoefficients, const Vec3& lightColor, const Vec3& lightPosition, const Vec3& eyePosition) {
}
