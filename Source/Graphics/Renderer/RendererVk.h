...
public:
	RendererVk(const int& width, const int& height);
	virtual ~RendererVk();
	...
	SpatialBoundVisual3* createVisualSpatial(const Mesh& mesh, const Material& material);
	VisualEffect* createVisualEffect(const Effect& effect);
	void draw(SpatialBoundVisual3* visual, VisualEffect* visualEffect, const Vec3& eyePosition, const std::vector<SpatialLight*>& spatialLights);
	...
	void draw(const Bound3& bound, VisualEffect* visualEffect, VisualMaterial* visualMaterial, const Mat4& worldMatrix, const Mat4& viewMatrix, const Mat4& viewProjectionMatrix, const Vec3& lightCoefficients, const Vec3& lightColor, const Vec3& lightPosition, const Vec3& eyePosition);
