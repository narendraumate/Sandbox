...
SpatialBoundVisual3* RendererVk::createVisualSpatial(const Mesh& mesh, const Material& material) {
	return new SpatialBoundVisualVk3(mesh, material);
}
...
void RendererVk::draw(SpatialBoundVisual3* visual, VisualEffect* visualEffect, const Vec3& eyePosition, const std::vector<SpatialLight*>& spatialLights) {
	...
	if (vkVisualMaterial && vkVisualEffect && vkVertexBuffer && visualIndexBufferCount) {
		...
		vkVisualEffect->setMatrix3fv("worldViewNorm", 1, false, (float*)&(visual->getWorldViewNormMatrix()));
				...
				if (vkIndexBuffer) {
					...
					switch (vkIndexBuffer->getType()) {
						...
						case IndexBuffer::Quadrilateral:
							////vkDrawElements(GL_QUADS, static_cast<int>(vkIndexBuffer->getIndexCount()), GL_UNSIGNED_INT, BUFFER_OFFSET(0));
							break;
						case IndexBuffer::Face:
							////vkMultiDrawElements(GL_TRIANGLE_STRIP, (GLsizei*)vkIndexBuffer->getCounts(), GL_UNSIGNED_INT, (const GLvoid**)vkIndexBuffer->getOffsets(), static_cast<int>(vkIndexBuffer->getPrimitiveCount()));
							break;
						default:
							break;
					}
...
VisualScreenSpace* RendererVk::createVisualScreenSpace(const Effect& effect) {
	return new VkVisualScreenSpace(effect);
}
...
void RendererVk::destroyVisualScreenSpace(VisualScreenSpace* visualScreenSpace) {
	VkVisualScreenSpace* vkVisualScreenSpace = dynamic_cast<VkVisualScreenSpace*>(visualScreenSpace);
	delete vkVisualScreenSpace;
}

void RendererVk::draw(const Bound3& bound, VisualEffect* visualEffect, VisualMaterial* visualMaterial, const Mat4& worldMatrix, const Mat4& viewMatrix, const Mat4& viewProjectionMatrix, const Vec3& lightCoefficients, const Vec3& lightColor, const Vec3& lightPosition, const Vec3& eyePosition) {
	...
	if (true) {
		...
		vkVisualEffect->set3fv("eyePosition", 1, eyePosition.data);
		// Set World View Norm Matrix.
		vkVisualEffect->setMatrix3fv("worldViewNorm", 1, false, (float*)&(worldViewNormMatrix));
		// Set World View Projection Matrix.
	...
	// Draw your self.
	{
		...
		IndexBufferVk vkIndexBuffer(IndexBufferVk::Edge, indices);
		vkVertexBuffer.bind();
		vkIndexBuffer.bind();
		////vkDrawElements(GL_LINES, static_cast<int>(vkIndexBuffer.getIndexCount()), GL_UNSIGNED_INT, BUFFER_OFFSET(0));
		vkIndexBuffer.unbind();
