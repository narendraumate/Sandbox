...
TEST_CASE("Projection") {
    auto getProjection = [](bool forward, bool finite) {
        ProjectionType type = Perspective;
        ProjectionHand hand = Right;
        ProjectionSize size = X2Y2Z1;
        ...
        return Projection(type, hand, size, projectionParameter).getMatrix();
    };

    Mat4 m = Mat4::identity;

    m = getProjection(false, false); // reverse infinite
    ...
    m = getProjection(false, true); // reverse finite
    ...
    m = getProjection(true, false); // forward infinite
    ...
    m = getProjection(true, true); // forward finite
