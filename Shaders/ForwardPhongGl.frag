...
void main()
{
	...
	// Enable Bump Map
	// mat3 NTB = mat3(fNormal.rgb, fTangent.rgb, fBitangent.rgb);
	// vec3 N = normalize(NTB * (texture(bumpTextureSampler, vec2(fTexcoord.x, fTexcoord.y)).rgb * 2.0f - 1.0f));
    ...
    gl_FragDepth = fDepth = (rangeZeroToOne * (1.0f - (1.0f / fPosition.w))) + ((1 - rangeZeroToOne) * (1.0f / fPosition.w));
}
