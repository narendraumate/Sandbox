...
void main()
{
	...
	// Disable Bump Map
	vec3 N = normalize(worldViewNorm * vNormal.rgb);
	// Enable Bump Map
	// mat3 NTB = mat3(normalize(worldViewNorm * vNormal.rgb), normalize(worldViewNorm * vTangent.rgb), normalize(worldViewNorm * vBitangent.rgb));
	// vec3 N = normalize(NTB * (texture(bumpTextureSampler, vTexcoord).rgb * 2.0 - 1.0));
